      ...
      <section className="grid gap-px bg-line px-5 pb-24 md:mx-10 md:grid-cols-2 md:px-0">
        {articles.map((article, index) => (
          <Link
            ...
            className="group bg-surface p-6 sm:p-7 md:p-10"
          >
            <span className="font-mono text-[10px] tracking-widest text-accent">
              {String(index + 1).padStart(2, "0")}
            </span>
            <h2 className="mt-6 break-words font-display text-2xl group-hover:text-accent sm:mt-8 sm:text-3xl">
              {article.title}
            </h2>
            <p className="mt-4 leading-7 text-muted">{article.excerpt}</p>
            <p className="mt-6 font-mono text-[10px] uppercase tracking-widest text-muted sm:mt-8">
              {new Date(article.publishedAt).toLocaleDateString()}
            </p>
          </Link>
        ))}
        {!articles.length ? (
          <p className="bg-surface p-10 text-muted">No articles have been published yet.</p>
        ) : null}
      </section>
    </>
  );
